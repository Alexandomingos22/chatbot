...
def pergunta(request, id):
    titulo = "Cadastro de Perguntas e Respostas"
    pergunta = Pergunta.objects.filter(id=id)
    global codeUser
...
def edicao(request, id):
    titulo = "Ediçao de Perguntas e Respostas"
    global codeUser
...
def delecao(request, id):
    titulo = "Deleçao de Perguntas e Respostas"
    global codeUser
    todas = Pergunta.objects.filter(code_user=codeUser)
    pergunta = Pergunta.objects.get(id=id)
    usuario = Usuario.objects.filter(code=codeUser)
...
def questao(request, code_user, code_before, question):
    ...
    for token in tokens:
        ...
        if len(parts) == 8:
            try:
                res = pycep_correios.get_address_from_cep(parts)
            ...
            if res != False:
                cep = parts.strip()
                estado = res['uf'].strip()                                                                                             #localidade[res.localidade.index('/'):].replace('/','').strip()
                cidade = res['cidade'].strip()                                                                                         #res.localidade[:res.localidade.index('/')].strip()
